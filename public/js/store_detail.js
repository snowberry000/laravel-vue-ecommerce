app.controller('stores_detail', ['$scope', '$http', '$timeout', function ($scope, $http, $timeout) {
  ...
  $(document).on('click', '.value-changer', function () {
    ...
    if ($(this).attr('data-val') == 'add') {
      if (selectedOrder.item_count < 20) {
        selectedOrder.item_count = parseInt(selectedOrder.item_count) + 1;
    ...
    $scope.order_data.items = $scope.order_data.items.map(item => {
      if (item.item_id === parseInt(item_id))
  ...
  $(document).on('click', '.btn-add-cart', function() {
    ...
    $http.post(url_item, {
      ...
      var menu_item = response.data.menu_item;
      var menu_item_price;
      if (response.data.menu_item.offer_price != 0) {
        menu_item_price = response.data.menu_item.offer_price;
      }
      else {
        menu_item_price = response.data.menu_item.price;
      }
